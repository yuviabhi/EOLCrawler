 ...
 *        
 */
class CSVImportHandler extends ImportHandler {
	private $headers = array ();
	private $id_col_index = 0;
	public function set_configuration(array $headers, int $id_col_index) {
		$this->headers = $headers;
		$this->id_col_index = $id_col_index;
	}
	public function process_src_document($document_row) {
		$ndli_doc_id = $this->generate_ndli_document_id ( $document_row [$this->id_col_index] );
		if ($ndli_doc_id) {
			$document = new NDLIDocument ( $ndli_doc_id );
			$len = count ( $document_row );
			for($i = 0; $i < $len; $i ++) {
				if ($document_row [$i]) {
					$document->add_metadata ( $this->headers [$i], explode ( "||", $document_row [$i] ) );
				}
			}
			if ($collection = $document->get_metadata ( "collection" )) {
				$document->set_collection ( current ( $collection ) );
			}
	...
	protected function generate_ndli_document_id(string $src_doc_id) {
		return preg_replace ( "/(.*?)(hdl\.handle\.net)\/(.*)/", "$3", $src_doc_id );
	}
}
class _EPrints {
			...
			if (isset ( $item->related_url )) {
				foreach ( $item->related_url as $url ) {
					if (isset ( $url->type ) && isset ( $url->url )) {
						...
						$document->add_metadata ( "related_url.OBJECT", json_encode ( $url, JSON_UNESCAPED_UNICODE ) );
					} else {
